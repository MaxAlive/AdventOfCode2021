...
data = [[int(risk_lvl) for risk_lvl in line] for line in data]
data_copy = [line[:] for line in data]

for i in range(4):
    for j, line in enumerate(data_copy):
        data[j].extend([(value+i)%9 + 1 for value in line])

data_copy = [line[:] for line in data]

for i in range(4):
    for line in data_copy:
        data.append([(value+i)%9 + 1 for value in line])


graph = GridWithWeights(data)
came_from, cost_so_far = a_star_search(graph, (0,0), (499,499))

print(cost_so_far[499,499])
